...
import '../divisionresponsable/styles/dashboardpage.css';
...
const Statisticepardivision = () => {
  ...
  return (
    ...
    <div className="statsGrid">
        ...
        <div className="statCard">
          <p className="statLabel">Terminé</p>
          <h2 className="statValueGreen">
            {statuses.find(s => s.statut === 'terminé')?.count || 0}
          </h2>
        </div>
        <div className="statCard">
          <p className="statLabel">annulé</p>
          <h2 className="statValueGreen">
            {statuses.find(s => s.statut === 'annulé')?.count || 0}
          </h2>
        </div>
       ...
       <div className="mainContent">
        <div className="chartCard">
          <h3 className="sectionTitle">Répartition des tâches</h3>
         ...
         <div className="activityCard">
          <h3 className="sectionTitle">Activité récente</h3>
      ...
      <div className="documentsCard">
        <h3 className="sectionTitle">Documents récents</h3>
        <div className="documentGrid">
          {documents.map((document, index) => (
            <div key={`document-${document.document_id}-${index}`} className="documentItem">
              <p>{tasks[index]?.task_name || 'Tâche inconnue'}</p>
              <span className="documentIcon">📄</span>
              <div>
                <a href={`http://localhost:8000/storage/${document.document_path}`} target='_blank' rel="noopener noreferrer">Télécharger le document</a> 
              </div>
            </div>
          ))}
        </div>
