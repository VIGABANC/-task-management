...
export default function TaskManagement({ user }) {
  ...
  const handleActivateTask = async (taskId) => {
    ...
    try {
      ...
      console.error('Erreur lors de la mise à jour du statut de la tâche:', error.response ? error.response.data : error.message);
      setSnackbar({
        open: true,
        message: `Échec de la mise à jour du statut de la tâche: ${error.response?.data?.message || error.message}`,
        severity: 'error'
      });
    }
  };
  ...
  // Filter tasks by status (only en attente tasks)
    ...
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <Typography variant="h6" color="error">
          Veuillez vous connecter pour voir les tâches
        </Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">
          Tâches en attente pour {user.division_nom}
        </Typography>
      </Box>
      ...
      <Grid container spacing={3}>
        {filteredTasks.map((task) => (
          <Grid item xs={{ xs: 12, sm: 6, md: 4 }} key={task.task_id}>
            <Card
              sx={{
                height: '100%',
                display: 'flex',
